...
8. **Stats** (`bin/inc/stats.sh`): Reads parsed CSV files under `results/` and reports per-algorithm row counts, failed row counts, average `Cut`, and average `Time`. `mkexp2 stats --json` emits structured data for the web Stats tab. Successful averages ignore rows whose `Failed` column is truthy.
